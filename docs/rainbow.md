...
This is a very simple example of building a rainbow inspired by the
[RaspberryJuice](https://dev.bukkit.org/projects/raspberryjuice) plugin.
In this example, we'll build a rainbow near current player and immediately watch the rainbow.

## Build rainbow with R graphics

First, you'll need to draw a rainbow with the R base plot system.

- Generate rainbow data
- Prepare rainbow colors
- Draw a rainbow with R base plot

It is possible to draw a rainbow with other R plot systems like
...
1. Configure rainbow height, colors, and items.
1. Build a rainbow, referring to current player location and configured items.
1. Finally, watch the rainbow by moving a bit away from the current player position.
...
![image of rainbow in minecraft](figure/rainbow.png)
