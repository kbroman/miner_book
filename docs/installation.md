...
`nano`, or you could install
[Notepad++](https://notepad-plus-plus.org/downloads/).

Open git bash, and type the following, to make and enter a directory
...
You might check that java is installed correctly.
...
java -jar -Xms1024M -Xmx4000M spigot-26.1.2.jar nogui
```
...
Now we download the raspberry juice plugin.
...
The rest follows the Docker instructions, run using git bash. As with
the Mac, you may need `sudo` with the docker commands, and you may
need to enable `sudo` in the developer settings.
...
Now open git bash and build the docker container:
